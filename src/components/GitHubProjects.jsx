...
const GitHubProjects = () => {
  ...
  useEffect(() => {
    const fetchGitHubProjects = async () => {
      try {
        ...
        const formattedProjects = filteredRepos.map((repo, index) => ({
          id: `project-${index + 1}`,
          content: repo.description || "No description provided.",
          name: repo.name,
          title: new Date(repo.created_at).toLocaleDateString(),
          link: repo.html_url,
        }));
  ...
  return (
    ...
    >
      ...
      <div>
        <h4 className={`${styles.heading2} p-20 text-center`}>Projects from GitHub</h4>
